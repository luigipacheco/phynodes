...
Install the `animaquina_mqttouch` folder via **Edit → Preferences → Add-ons →
Install**, or drop it in your Blender extensions/addons folder and enable it.

## Usage

1. Open a **Node Editor** and switch the tree-type dropdown (header) to
   **MQTTouch**. Create a new node group.
2. In the **N-panel → MQTTouch** tab, set the broker host / port / topic prefix
   and click **Connect**.
3. **Add → MQTTouch** to drop nodes. Wire them up.
...
- `tree.py` — the `MQTTouch` node tree + generic Variant socket.
- `nodes/` — one module per node.
- `ui.py` — Add menu + broker N-panel.

## Status

Beta / MVP. Planned next: Clamp, Compare, Switch, Array, Color, JSON nodes;
optional per-node broker override; vendored `paho-mqtt` wheel for the extension.
