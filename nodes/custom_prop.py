...
# driver-friendly property — e.g. scene["myx"] — of a chosen type, exactly like
# a hand-made custom property. Use a Single Property driver variable on the path
# from "Copy Var Path".
...
class AQCustomPropertyNode(AQBaseNode, Node):
    ...
    def _ensure_property(self, context=None):
        ...
        try:
            ...
            if self.prop_type == "BOOL":
                ...
                ui.update(description="PhyNodes")
    ...
    def evaluate_sink(self):
        ...
        if not tgt or not name:
            return
        val = self._coerce(self.get_input("Value", 0.0))
        key = repr(val)
